...
const platformdemoAPI = axios.create({
  baseURL: "https://platformdemo-2-0-7-40390-prod.herokuapp.com",
  headers: {
...
function modules_articles_article_list(payload) {
  return platformdemoAPI.get(`/modules/articles/article/`);
}

function modules_articles_article_create(payload) {
  return platformdemoAPI.post(`/modules/articles/article/`, payload.data);
}
...
function modules_articles_article_retrieve(payload) {
  return platformdemoAPI.get(`/modules/articles/article/${payload.id}/`);
}

function modules_articles_article_update(payload) {
  return platformdemoAPI.put(`/modules/articles/article/${payload.id}/`, payload.data);
}

function modules_articles_article_partial_update(payload) {
  return platformdemoAPI.patch(`/modules/articles/article/${payload.id}/`, payload.data);
}

function modules_articles_article_destroy(payload) {
  return platformdemoAPI.delete(`/modules/articles/article/${payload.id}/`);
}
